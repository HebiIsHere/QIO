...
- 召回层可插拔：BM25（纯 Python，默认档，无本地模型依赖）；ONNX/远程嵌入接口预留
- 启动自检选档：远程嵌入（配置）→ ONNX（库+模型可用）→ BM25（总是可用）；无可用召回时降级规则层（rules-only）
- 精排层：可选，默认关闭
...
- 封块策略：消息数（默认 30）或 token 阈值触发；两步行：append 返回关闭信号 → turn 结束时注入摘要生成器执行关闭
...
- 显式反馈纠错 → revoke（v1）；Dreaming/隐式反馈（v1.5）
### 4.11 图导航层（M8）
...
- 注入预算：知识域 2–4K token（按记忆强度滑块线性插值，优先填充）；记忆域相关性阈值驱动（默认 0.05）；硬上限 = 上下文 × 10% 动态
...
- 显式反馈纠错 → revoked/更新（v1）；隐式反馈 + Dreaming 离线整理 + 轨迹→工具候选（v1.5）
...
- 函数型 + 子 agent 型并列；同步/异步；子 agent 独立循环/模型/BYOK Key 引用，默认 untrusted，产出不自动进知识域
- 生命周期：生成前解释 → 交叉测试（确定性断言为主 + LLM 评估为辅）→ 双段审批（创建 + 凭据授权）→ 沙箱 → 并列注册
- 沙箱：默认受限子进程执行器 + Docker 可选（无显卡低配机不常驻 Docker）
- 工具定义可脱离主循环自主运行（子 agent 化），拥有自己的 apikey 引用
