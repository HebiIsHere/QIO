...
| 临时布局 | 新增 `frontend/src/planet/layoutSlots.ts`：**打开时在整个球面随机铺开**（`spreadPositions`，稳定种子可复现）；**拖动摇动期间新话题在球体背面随机落点**（`randomBackPosition`，避开已有方向、保持角间距）；静止时位置完全不动。`nodes.meta.layout` 旧坐标保留兼容但不再是核心语义 |
| 相机 | 纵向限位 35°~145°（`POLAR_LIMIT`）：拖不到极点，横向旋转不会在极点附近退化；程序性相机移动遵守同一条限位 |
...
| 开局像一条项链 | 打开时用固定极角环带布局，所有话题挤在赤道附近 | 改成整个球面随机铺开（保留最小角间距与极冠留白），话题分布在球体各处 |
| 纵向拖到极点后「怎么拖都不动」 | OrbitControls 没有纵向限位，极角接近 0/π 时方位角失去意义 | 加纵向限位 35°~145°，程序性相机移动走同一个 `clampPolar` |
...
- **Topic 如何进入展示**：后端 `_sequence()` 用「稳定哈希打底 + 少量近期活跃加权」生成确定性顺序，第一圈把当前所在话题排到首位；前端按游标取批，打开时在整个球面随机铺开窗口，之后进入的话题在**球体背面随机落点**（与窗口里已有话题保持最小角间距）。
...
| `frontend/src/planet/__tests__/layoutSlots.test.ts`（新增用例） | `spreadPositions` 同 seed 稳定 / 不重叠 / 铺开到整个球面（|y| 幅度 > 1.3）；`clampPolar` 把极点压回限位、限位内原样保留 | 通过 |
...
| 1b 初始铺开 | 开局是不是「一条环带」 | 运行。16 个方向的 \|y\| 跨度 0.02~0.88（环带布局不会超过 0.6），说明话题铺满整个球面 |
...
| 纵向限位 | 用力纵向拖动会不会拖到极点 | 运行。纵向拖 700px 后相机极角 35.0°（限位 35°~145°），不会再出现「怎么拖都不动」 |
...
- **随机铺开的观感没有做过对比**：开局随机铺开与旧的环带布局只做了单次截图比较，没有做「多个 seed 下的最差情形」抽查（例如某个 seed 恰好让两三个点靠得较近）。
