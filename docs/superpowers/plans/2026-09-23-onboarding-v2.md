...
- ✅ Task 1 知识来源/范围/时间可见 + 已结束降权 —— `eef7a29`（后端全量 pytest 通过）
- ✅ Task 2 话题结束语义与"已结束"分组数据 —— 并入 `dba15f9`
- ✅ Task 3 引导一次性提交 + 自我锚点（改名迁移、曾用名、合并）—— 并入 `dba15f9`
- ✅ Task 4 版本弹窗改为一次性跨越 0.1.7 —— `085108b`
- ✅ Task 5 七步向导（本地草稿、核对清单、追问）+ 追问端点 —— `c0abf17`（前端 664 passed / vue-tsc 0）
- ✅ Task 6 知识页（来源/范围/时间、来源筛选、标记结束/恢复）与星球"已结束"分组 —— `c561e96`（前端 670 passed / vue-tsc 0）
- ✅ Task 7 预览与验收截图 —— `scripts/ui-catalog/onboarding-preview.mjs`（七步逐页 + 提交后落库核对）
